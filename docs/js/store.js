...
export async function saveResultats(annee, code, data) {
  const id = `${annee}_${code}`;
  if (DEMO) { localStorage.setItem(`res_${id}`, JSON.stringify(data)); return; }
  await fs.setDoc(fs.doc(db, "resultats", id), data);
}

export async function deleteResultats(annee, code) {
  const id = `${annee}_${code}`;
  if (DEMO) { localStorage.removeItem(`res_${id}`); return; }
  await fs.deleteDoc(fs.doc(db, "resultats", id));
}

// Supprime une remontée et tout son historique (utilisé par la réinitialisation des données).
export async function deleteSaisieComplete(code) {
  const id = `${CAMPAGNE}_${code}`;
  if (DEMO) {
  ...
  await fs.deleteDoc(fs.doc(db, "saisies", id));
  const hist = await loadHistorique(code);
  await Promise.all(hist.map(h => fs.deleteDoc(fs.doc(db, "saisies_historique", `${id}_${h.timestamp.replace(/[:.]/g, "-")}`))));
}
