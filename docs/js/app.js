...
async function viewResultats() {
  ...
  <div class="card">
    ${estConsolidable ? `<div class="toolbar"><span class="muted">${nbSaisi}/${unites.length} ${esc(titreEnfants)} remontées</span>
      ...
      <span class="spacer"></span><button class="btn btn--secondary" id="btn-appliquer">Appliquer le consolidé comme ambition</button></div>` : ""}
  ...
  if (estConsolidable) {
    $("#btn-appliquer")?.addEventListener("click", async () => {
      const cible = (await loadSaisie(ctx.code)) || { qualitatif: {}, ambitions: {}, ressources: {} };
      let applique = 0;
      for (const i of INDICATEURS.filter(x => x.fourchette)) {
        const c = consolider(i.id, unites);
        if (c) { cible.ambitions[i.id] = { min: c.min, max: c.max }; applique++; }
      }
      ...
      cible.nomStructure = ctx.nom; cible.niveau = ctx.niveau; cible.region = ctx.region || null; cible.codeDD = ctx.codeDD || null;
      await saveSaisie(ctx.code, cible, email);
...
async function renderRemontee() {
  ...
  function render() {
    ...
    const bloc = (id, titre, contenu, ouvert) => `<details class="card fold" data-bloc="${esc(titre)}" ${ouvert ? "open" : ""}>
      <summary>${esc(titre)}<span class="fold-arrow">▾</span></summary>
    ...
    <div class="sticky-context">
      <span>📍 <b>${esc(ctx.nom)}</b></span><span class="sep">·</span><span>Revue d'impact</span><span class="sep">·</span><span id="bloc-actif">${esc(DOMAINES.find(d => d.id === domaine)?.nom || "")}</span>
    </div>
    <div class="help"><b>Aide à la saisie.</b> Trois blocs par domaine, à déplier selon vos besoins : le <b>bilan ${ANNEE_BILAN}</b> (réussites et axes d'amélioration), la <b>stratégie et les leviers</b>, puis les <b>ambitions chiffrées</b> en fourchette Min/Max (± 5 % pour les volumes ; ± 0,5 pt sur TAE, ± 2 pts sur DYN ou SATIS). Les textes sont limités à ${MAX_CHARS} caractères. ${unitesEnfants.length ? `Le bouton <b>"Détail par ${esc(labelEnfants)}"</b> affiche, pour chaque question, les réponses déjà remontées par vos unités rattachées. ` : ""}<b>La remontée n'étant pas nominative de bout en bout, chaque enregistrement est historisé avec l'email de son auteur</b> ; seul le compte administrateur peut consulter et restaurer les versions antérieures.</div>
    <div class="tabs">${DOMAINES.map(d => `<button class="tab ${d.id === domaine ? "active" : ""}" data-d="${d.id}">${esc(d.nom)}</button>`).join("")}</div>
    ${bloc("bilan", `Bilan ${ANNEE_BILAN}`, t.bilan.map(i => area("bilan", i)).join(""), true)}
    ${t.strategie.length ? bloc("strategie", "Stratégie", t.strategie.map(i => area("strategie", i)).join("")) : ""}
    ${t.leviers.length ? bloc("leviers", "Leviers d'action", t.leviers.map(i => area("leviers", i)).join("")) : ""}
    ${t.ressources.length ? bloc("ressources", "Ressources", `<div class="grid grid-3">${t.ressources.map(i => `
      <div class="field"><label>${esc(i.label)}</label><input type="number" min="0" step="0.5" data-r="${i.id}" value="${esc(r[i.id] ?? "")}"></div>`).join("")}</div>`) : ""}
    ${inds.length ? bloc("ambitions", `Ambitions chiffrées ${CAMPAGNE}`, `
...
// ---- Bloc Consolidation (réussites/difficultés + cibles nationales — le tableau numérique est dans l'onglet Résultats et ambitions) ----
async function renderConsolidation() {
  ...
  <div class="help">Les ambitions chiffrées consolidées et l'écart à l'ambition propre de la structure sont désormais dans l'onglet <b>Résultats et ambitions</b>. Cette section rassemble les réussites/difficultés remontées${cibles !== null ? ", ainsi que les cibles nationales imposées par les financeurs" : ""}.
  ...
  ${cibles !== null ? `<div class="card">
    ...
    <p class="muted">Saisies par la Direction de la Performance (taux en décimal, ex. 0,47 ; volumes en nombre). Comparées à l'ambition consolidée dans l'onglet Résultats et ambitions.</p>
