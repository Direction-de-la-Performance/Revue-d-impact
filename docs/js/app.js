...
import { initStore, getReferentiel, loadSaisie, saveSaisie, loadSaisies, loadResultats, loadCibles, saveCibles, DEMO } from "./store.js";
...
function renderCtx() {
  ...
  $("#ctx-structure").innerHTML = ctx.code
    ? `<b>${esc(ctx.nom)}</b>${lbl[ctx.niveau] || ""}${DEMO ? ' · <span class="badge badge--jaune">mode démo</span>' : ""}`
