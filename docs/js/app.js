...
async function viewAccueil() {
  ...
  $("#btn-donnees-fictives")?.addEventListener("click", () => {
    ...
    demanderConfirmation(
      ...
      `Cela va créer des résultats et une remontée fictifs pour <b>${esc(ctx.nom)}</b> et ses unités directement rattachées, en <b>écrasant toute donnée déjà existante</b> à ces mêmes codes. Continuer ?`,
      "Générer",
...
async function genererDonneesFictives() {
  ...
  for (const code of codes) {
    await saveResultats(ANNEE_BILAN, code, demoResultats(code));
  ...
  toast(`Données fictives générées pour ${codes.length} structure(s).`);
}
...
async function calculerAvancement() {
  ...
  if (ctx.niveau === "REG") {
    ...
    dds.forEach(d => counts[statutSaisie(map[d.code])]++);
  ...
  if (ctx.niveau === "NAT") {
    ...
    REF.regions.forEach(r => counts[statutSaisie(map["REG_" + r])]++);
...
async function viewResultats() {
  ...
  for (const i of INDICATEURS) {
    ...
    rows += `<tr>
      <td><b>${esc(i.label)}</b>${i.sensInverse ? ' <span class="muted" title="Une baisse est une amélioration">▼</span>' : ""}${i.aide ? ` <span class="info-ico" title="${esc(i.aide)}">ⓘ</span>` : ""}<br><span class="muted">${esc(i.nom)}</span></td>
      ...
      <td class="num ${ambCellClass}">${valAmb != null ? fmtVal(i, valAmb) : "-"}</td>
      ${estConsolidable ? `
  ...
  <div class="card">
    ...
    <div style="overflow-x:auto"><table>
      <thead><tr>
        ...
        ${estConsolidable ? `<th class="num">Consolidé</th><th class="num">Couverture</th><th class="num">Écart ambition/consolidé</th>` : ""}
      </tr></thead>
      <tbody>${rows}</tbody>
    </table></div>
  </div>
  ...
  function render() {
    ...
    const ambitionsContenu = () => {
      const rows = inds.map(i => {
        ...
        return `<tr>
          <td><b>${esc(i.label)}</b>${i.aide ? ` <span class="info-ico" title="${esc(i.aide)}">ⓘ</span>` : ""}<br><span class="muted">${esc(i.nom)}${i.sensInverse ? " (une baisse est une amélioration)" : ""}</span></td>
          <td class="num">${v1 != null ? fmtVal(i, v1) : "-"}</td>
      ...
      <div style="overflow-x:auto"><table>
        <thead><tr>
          ...
          <th class="num">Évolution / ${ANNEE_A2}</th>
          <th class="num">Cible nationale ${CAMPAGNE}</th>
          <th>Votre ambition ${CAMPAGNE}</th>
          ${consolidable ? `<th class="num">Consolidé</th><th class="num">Couverture</th><th class="num">Écart</th>` : ""}
        </tr></thead>
        <tbody>${rows}</tbody>
      </table></div>
      ${emailSaveHtml()}`;
