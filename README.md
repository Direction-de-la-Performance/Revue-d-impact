...
2. **Résultats et ambitions** : pour chaque indicateur : résultat A-1, taux d'atteinte de l'objectif A-1, évolution vs A-2, écart à la moyenne pondérée des agences les plus proches (maille agence uniquement), cible nationale de la campagne, ambition saisie par la structure ; et, à partir de la maille Direction Départementale, dans le **même tableau** : la valeur consolidée de ses unités rattachées, leur couverture, et l'écart entre l'ambition propre de la structure et ce consolidé (texte en rouge en cas de divergence). Un bouton **"Appliquer le consolidé comme ambition"** permet d'aligner en un clic l'ambition de la structure sur la consolidation de ses unités. Un bouton **"Télécharger le rapport PDF"** génère un rapport au format A4 portrait : une première page Résultats et ambitions, puis une page par domaine (Bilan, Stratégie, Leviers, Ressources — sans les ambitions, déjà en page 1). Les pages sont de taille fixe : un texte trop long y est tronqué (il reste consultable en intégralité dans l'outil).
3. **Revue d'impact** : remontée des réussites, difficultés et ambitions (une valeur par indicateur, accompagnée du résultat de l'année de bilan, du taux d'atteinte, de l'évolution et de la cible nationale), historisée à chaque enregistrement ; et, à partir du niveau Direction Départementale, consolidation pondérée ascendante. Les blocs (Bilan, Stratégie, Leviers, Ressources, Ambitions) sont pliables/dépliables, chacun affichant son niveau de complétude. Un bandeau flottant garde en permanence visibles la structure sélectionnée, l'onglet et le bloc actif. À partir de la maille DD, chaque question propose un bouton « Détail » listant les réponses des unités rattachées, et le bloc Ambitions affiche, dans le même tableau que la saisie, la consolidation chiffrée de ces unités avec un bouton pour l'adopter directement. Les ressources (ETP) sont également consolidées (somme).
...
7. Importer les remontées déjà saisies (bilan, ambitions, ressources extraits du classeur Excel) : ouvrir `docs/admin-import-saisies.html`, saisir le mot de passe, sélectionner `data-confidentielle/saisies_2026.json`, cliquer Importer.
   Cet outil peut aussi servir à **réutiliser un exercice réel antérieur comme données fictives** pour la campagne en cours (utile pour visualiser le rendu sur des données réalistes) : les cases "Marquer l'auteur comme données fictives" et "Générer aussi des résultats fictifs" sont cochées par défaut dans ce but — l'import écrit directement dans la campagne actuellement configurée (`CAMPAGNE` dans `config.js`), quel que soit le millésime figurant dans le nom du fichier.
...
- **Aide à la lecture par indicateur** : une icône ⓘ à côté de chaque indicateur (tableaux Résultats et ambitions, Revue d'impact) affiche au survol une explication de ce qu'il mesure.
- **Boutons d'accès rapide sur l'Accueil** : en plus d'"Accéder aux résultats", des boutons permettent d'aller directement à la Revue d'impact ou de télécharger le rapport PDF pour la structure sélectionnée dans les menus déroulants.
- **Structure cliquable en en-tête** : cliquer sur le nom de la structure (en haut à droite) ramène à l'Accueil pour en choisir une autre.
- **Confirmation avant de quitter la Revue d'impact** : si des modifications n'ont pas été enregistrées (saisie, changement d'onglet, changement de structure), une fenêtre propose de les enregistrer (en ouvrant et en pointant le champ email) ou de les abandonner.
- **Données fictives (admin)** : sur l'Accueil, deux actions permettent de générer des résultats et une remontée fictifs pour la structure sélectionnée et ses unités directement rattachées (utile pour visualiser le rendu), ou de réinitialiser (supprimer) ces mêmes données. Les deux actions demandent confirmation, la seconde étant irréversible. Les entrées d'historique issues de données fictives sont signalées par un badge distinct.
- **Détail de l'avancement des remontées** : dans l'onglet Résultats et ambitions, cliquer sur un des compteurs (complètes/partielles/vides) affiche la liste des structures concernées.
